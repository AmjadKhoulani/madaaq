...
<div class="max-w-5xl mx-auto space-y-6" x-data="towerForm">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">تحديث معلومات وتجهيزات البرج: {{ $tower->name }}</p>
        </div>
        <div class="flex gap-3">
             <a href="{{ route('network.towers.show', $tower) }}" class="px-5 py-2.5 text-blue-600 bg-blue-50 border border-blue-200 hover:bg-blue-100 font-medium rounded-xl transition shadow-sm">
                👁️ عرض البرج
            </a>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <label class="cursor-pointer group">
                            <input type="radio" name="type" value="tower" class="peer sr-only" {{ old('type', $tower->type) == 'tower' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="type" value="building" class="peer sr-only" {{ old('type', $tower->type) == 'building' ? 'checked' : '' }}>
                            <div class="text-center py-4 border-2 border-gray-100 rounded-2xl peer-checked:border-blue-500 peer-checked:bg-blue-50/50 group-hover:border-blue-200 transition bg-white">
                                <div class="text-2xl mb-2">🏢</div>
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="type" value="pole" class="peer sr-only" {{ old('type', $tower->type) == 'pole' ? 'checked' : '' }}>
                            <div class="text-center py-4 border-2 border-gray-100 rounded-2xl peer-checked:border-blue-500 peer-checked:bg-blue-50/50 group-hover:border-blue-200 transition bg-white">
                                <div class="text-2xl mb-2">📍</div>
                ...
                <div>
                    ...
                    <select name="city" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach(['Damascus' => 'دمشق', 'Aleppo' => 'حلب', 'Homs' => 'حمص', 'Latakia' => 'اللاذقية', 'Hama' => 'حماة', 'Tartus' => 'طرطوس', 'Idlib' => 'إدلب', 'Daraa' => 'درعا', 'Suwayda' => 'السويداء', 'Rif Dimashq' => 'ريف دمشق'] as $key => $val)
                            <option value="{{ $key }}" {{ old('city', $tower->city) == $key ? 'selected' : '' }}>{{ $val }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">المنطقة / الحي</label>
                    <input type="text" name="district" value="{{ old('district', $tower->district) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500">
                </div>
                
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 mb-2">العنوان التفصيلي</label>
                    <input type="text" name="location" value="{{ old('location', $tower->location) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500" placeholder="الشارع، العلامات المميزة...">
                </div>

                <!-- Specs -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">الارتفاع (متر)</label>
                    <input type="number" name="height" value="{{ old('height', $tower->height) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="border border-gray-200 rounded-xl p-4 hover:border-blue-300 transition" :class="{'bg-blue-50 border-blue-300': hasSolar, 'bg-gray-50': !hasSolar}">
                    ...
                    <div x-show="hasSolar" x-collapse>
                        <div class="grid grid-cols-2 gap-3 mt-2">
                            <input type="number" name="solar_panels_count" value="{{ old('solar_panels_count', $tower->solar_panels_count) }}" placeholder="العدد" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                            <input type="number" name="solar_panel_wattage" value="{{ old('solar_panel_wattage', $tower->solar_panel_wattage) }}" placeholder="القدرة (واط)" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="border border-gray-200 rounded-xl p-4 hover:border-blue-300 transition" :class="{'bg-blue-50 border-blue-300': hasGenerator, 'bg-gray-50': !hasGenerator}">
                    ...
                    <div x-show="hasGenerator" x-collapse>
                        <input type="text" name="generator_capacity" value="{{ old('generator_capacity', $tower->generator_capacity) }}" placeholder="القدرة (مثال: 10 KVA)" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                    </div>
                </div>
                ...
                <div class="border border-gray-200 rounded-xl p-4 bg-gray-50">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <input type="number" name="battery_count" value="{{ old('battery_count', $tower->battery_count) }}" placeholder="العدد" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                        <input type="text" name="battery_type" value="{{ old('battery_type', $tower->battery_type) }}" placeholder="النوع/السعة" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                     ...
                     <div class="relative">
                        <input type="number" step="0.01" name="structure_cost" value="{{ old('structure_cost', $tower->structure_cost) }}" class="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="monthly_rent" value="{{ old('monthly_rent', $tower->monthly_rent) }}" class="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 font-bold">{{ $currency }}</span>
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2" x-show="connectionType === 'cable' || connectionType === 'fiber'" x-transition>
                    ...
                    <input type="text" name="connection_port" value="{{ old('connection_port', $tower->connection_port) }}" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-left" dir="ltr" placeholder="ether1, sfp1...">
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div x-show="transmitterType === 'existing'">
                                <select name="transmitter_router_id" class="w-full px-3 py-2 bg-white border border-green-200 rounded-lg">
                                    ...
                                    <option value="{{ $router->id }}" {{ old('transmitter_router_id', $tower->transmitter_router_id) == $router->id ? 'selected' : '' }}>{{ $router->name }} ({{ $router->ip }})</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <!-- Receiver -->
                         <div class="space-y-4">
                            ...
                            <input type="text" name="receiver_name" value="{{ old('receiver_name', $tower->receiver_name) }}" placeholder="اسم الجهاز" class="w-full px-3 py-2 bg-white border border-green-200 rounded-lg">
                            <input type="text" name="receiver_ip" value="{{ old('receiver_ip', $tower->receiver_ip) }}" placeholder="IP Address" class="w-full px-3 py-2 bg-white border border-green-200 rounded-lg text-left" dir="ltr">
                            <select name="receiver_model_id" class="w-full px-3 py-2 bg-white border border-green-200 rounded-lg">
                                <option value="">الموديل...</option>
                                @foreach($deviceModels as $m)
                                <option value="{{ $m->id }}" {{ old('receiver_model_id', $tower->receiver_model_id) == $m->id ? 'selected' : '' }}>{{ $m->model_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('towerForm', () => ({
            ...
            hasSubscription: {{ old('has_ampere', $tower->has_ampere) ? 'true' : 'false' }},
