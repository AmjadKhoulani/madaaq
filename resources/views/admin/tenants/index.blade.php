...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold text-white">إدارة الشركات (Tenants)</h2>
            <p class="text-gray-400 mt-1">عرض وإدارة جميع مقدمي الخدمة المشتركين في المنصة</p>
        </div>
        <a href="{{ route('admin.tenants.create') }}" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors font-medium">
            ...
            <table class="w-full text-right">
                <thead>
                    <tr class="bg-gray-750 border-b border-gray-700 text-gray-400 text-sm uppercase">
                        ...
                        <th class="px-6 py-4 font-medium">النطاق (Domain)</th>
                        <th class="px-6 py-4 font-medium">الشركة</th>
                        <th class="px-6 py-4 font-medium">المالك (Owner)</th>
                        <th class="px-6 py-4 font-medium">النطاق (Domain)</th>
                    ...
                    <tr class="hover:bg-gray-700/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-gray-700 flex items-center justify-center text-lg font-bold text-gray-300">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @endphp
                            @if($owner)
                                <div class="text-white text-sm">{{ $owner->name }}</div>
                                <div class="text-gray-500 text-xs">{{ $owner->email }}</div>
                            @else
                                <span class="text-gray-600 text-xs">-- لا يوجد مستخدمين --</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-indigo-400 font-mono text-sm">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span class="bg-red-500/10 text-red-400 px-2 py-1 rounded-full text-xs font-medium border border-red-500/20">معلق</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('admin.tenants.impersonate', $tenant->id) }}" method="POST" onsubmit="return confirm('تنبيه: سيتم تسجيل خروجك من لوحة الإدارة وتسجيل الدخول بحساب هذا العميل. للمتابعة اضغط موافق.')">
                                    @csrf
                                    <button type="submit" class="p-2 rounded-lg hover:bg-indigo-600/20 text-indigo-400 hover:text-indigo-300 transition-colors" title="تسجيل الدخول كمالك للشركة">
                                ...
                                <form action="{{ route('admin.tenants.toggle', $tenant->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من تغيير حالة الشركة؟')">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="p-2 rounded-lg hover:bg-gray-700 text-gray-400 hover:text-white transition-colors" title="{{ $tenant->status == 'active' ? 'إيقاف الشركة' : 'تفعيل الشركة' }}">
                                        @if($tenant->status == 'active')
                                ...
                                <a href="{{ route('admin.tenants.show', $tenant->id) }}" class="p-2 rounded-lg hover:bg-gray-700 text-gray-400 hover:text-white transition-colors" title="عرض التفاصيل">
